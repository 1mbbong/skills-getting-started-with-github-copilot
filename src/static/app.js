      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.innerHTML = `
          ...
          <div class="participants-section">
            ...
            ${details.participants.length > 0 ? `<ul class="participants-list">${details.participants.map(email => `<li><span class="participant-email">${email}</span><button class="participant-delete" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">✖</button></li>`).join("")}</ul>` : `<p class="no-participants">No one signed up yet.</p>`}
          </div>
        `;
